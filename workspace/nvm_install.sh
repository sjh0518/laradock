#!/usr/bin/env bash
#ADD>>> PUKYO.shenjianghui 2021-10-20 禅道0 docker构建因为国外下载资源失败导致构建失败，故先手工下载置于目录中提供构建成功率
...
nvm_source() {
  ...
  if [ "_$NVM_METHOD" = "_script-nvm-exec" ]; then
    ...
    if [ "_$NVM_METHOD" = "_script" ]; then
      ...
      #EDIT>>> PUKYO.shenjianghui 2021-10-20 禅道0 git克隆使用https几乎全失败，https改成git 速度很快
#      NVM_SOURCE_URL="https://github.com/${NVM_GITHUB_REPO}.git"
      NVM_SOURCE_URL="git://github.com/${NVM_GITHUB_REPO}.git"
      #<<<EDIT PUKYO.shenjianghui 2021-10-20 禅道0 git克隆使用https几乎全失败，https改成git 速度很快
    else
